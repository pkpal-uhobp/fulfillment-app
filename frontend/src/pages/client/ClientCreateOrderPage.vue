...
import { ArrowRight, PackagePlus, Plus, Trash2 } from '@lucide/vue'
  ...
  <section class="space-y-6">
    <div class="rounded-[2rem] border border-white/10 bg-white/[0.06] p-6 backdrop-blur sm:p-8">
      ...
      <p class="mt-4 max-w-3xl text-white/65">
        Выберите маршрут, дату и состав грузовых мест. Доступность дат теперь показана прямо внутри календаря.
      </p>
    </div>
    ...
    <form class="grid gap-6 xl:grid-cols-[1fr_380px]" novalidate @submit.prevent="submit">
      <div class="space-y-6">
        <section class="rounded-[2rem] bg-white p-6 text-[#07101f] shadow-[0_24px_70px_rgba(0,0,0,0.12)]">
          <div class="flex items-center gap-3">
            <div class="grid h-12 w-12 place-items-center rounded-2xl bg-[#ff4248]/10 text-[#ff4248]">
        ...
        <section class="rounded-[2rem] bg-white p-6 text-[#07101f] shadow-[0_24px_70px_rgba(0,0,0,0.12)]">
          ...
          <div class="mt-5 grid gap-3 sm:grid-cols-2">
            <button
              type="button"
              class="min-h-[140px] rounded-3xl border p-5 text-left transition"
              :class="form.handover_type === 'self_delivery' ? 'border-[#ff4248] bg-[#ff4248]/5 shadow-[0_0_0_4px_rgba(255,66,72,0.08)]' : 'border-slate-200 bg-slate-50 hover:border-slate-300'"
              @click="setHandoverType('self_delivery')"
            >
            ...
            <button
              type="button"
              class="min-h-[140px] rounded-3xl border p-5 text-left transition"
              :class="form.handover_type === 'pickup' ? 'border-[#ff4248] bg-[#ff4248]/5 shadow-[0_0_0_4px_rgba(255,66,72,0.08)]' : 'border-slate-200 bg-slate-50 hover:border-slate-300'"
              @click="setHandoverType('pickup')"
            >
          ...
          <div v-if="form.handover_type === 'self_delivery'" class="mt-6 grid items-start gap-4 lg:grid-cols-[1.15fr_.95fr_.95fr]">
            <label class="block lg:pt-[26px]">
              <span class="mb-2 block text-xs font-black uppercase tracking-[0.25em] text-slate-400">Дата сдачи</span>
              <DatePicker v-model="form.self_delivery_date" :min="today" :max="maxDate" :availability-days="calendarDays" :error="fieldErrors.self_delivery_date" placeholder="Выберите дату" />
            </label>

            <label class="block">
              <span class="mb-2 block text-xs font-black uppercase tracking-[0.25em] text-slate-400">С времени</span>
              <TimeSelect v-model="form.self_delivery_time_from" :from="8" :to="22" :error="fieldErrors.self_delivery_time" placeholder="С" />
            </label>

            <label class="block">
              <span class="mb-2 block text-xs font-black uppercase tracking-[0.25em] text-slate-400">До времени</span>
          ...
          <div v-else class="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <label class="block lg:pt-[26px]">
              ...
              <DatePicker v-model="form.pickup.pickup_date" :min="today" :max="maxDate" :availability-days="calendarDays" :error="fieldErrors.pickup_date" placeholder="Выберите дату" />
            </label>
      ...
      <aside class="space-y-6">
        ...
        <div class="rounded-[2rem] border border-white/10 bg-white/[0.06] p-6">
          <p class="text-xs font-black uppercase tracking-[0.35em] text-[#ff9ca0]">Календарь приёмки</p>
          <h3 class="mt-3 text-xl font-black">Доступность встроена в выбор даты</h3>
          <p class="mt-3 text-sm leading-6 text-white/55">
            Откройте поле даты: зелёная точка показывает доступный день, красная — закрытый день или исчерпанный лимит.
          </p>
        </div>
      </aside>
    </form>
