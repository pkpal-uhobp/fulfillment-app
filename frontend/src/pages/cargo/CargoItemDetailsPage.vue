...
import { apiFetch, getAccessToken, getCurrentUser } from '@/shared/api/http'
...
const roleBackLinks = {
  client: '/client/cargo-items',
  worker: '/worker/cargo-items',
  warehouse_worker: '/worker/cargo-items',
  logist: '/logist/cargo-items',
  logistician: '/logist/cargo-items',
  admin: '/admin',
}
...
function getPreviousCargoItemsPath() {
  if (typeof window === 'undefined') return ''

  const queryFrom = typeof route.query.from === 'string' ? route.query.from : ''
  if (allowedPreviousPaths.includes(normalizePath(queryFrom))) {
    return queryFrom
  }

  const historyBack = window.history.state?.back
  if (typeof historyBack === 'string' && allowedPreviousPaths.includes(normalizePath(historyBack))) {
    return historyBack
  }
...
const backLink = computed(() => {
  const previousCargoItemsPath = getPreviousCargoItemsPath()
  if (previousCargoItemsPath) return previousCargoItemsPath

  const role = String(getCurrentUser()?.role || '').toLowerCase()
  return roleBackLinks[role] || '/client/cargo-items'
})
...
async function loadCargoItem() {
  ...
  try {
    const payload = await apiFetch(`/cargo-items/scan?qr_code=${encodeURIComponent(qrCode.value)}`, {
      auth: true,
    })
    ...
    if (id) {
      try {
        const historyPayload = await apiFetch(`/cargo-items/${id}/history`, {
          auth: true,
        })
        ...
        history.value =
            ...
            historyPayload?.cargo_status_history ||
            historyPayload ||
            []
...
<template>
  <main class="min-h-screen bg-[#07101f] text-white">
    <div
        class="absolute inset-0 bg-[radial-gradient(circle_at_10%_0%,rgba(255,63,73,0.26),transparent_30%),radial-gradient(circle_at_95%_10%,rgba(20,184,166,0.24),transparent_28%),linear-gradient(135deg,#120814_0%,#07101f_55%,#06394b_100%)]"
    ></div>
    ...
    <section class="relative mx-auto w-[min(1320px,calc(100%-32px))] py-10 lg:py-16">
      <div class="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <RouterLink
            :to="backLink"
            class="inline-flex items-center gap-3 rounded-2xl border border-white/12 bg-white/5 px-5 py-3 text-sm font-black text-white/80 transition hover:bg-white/10 hover:text-white"
        >
        ...
        <RouterLink
            v-if="!isAuthorized"
            to="/login"
            class="inline-flex items-center gap-3 rounded-2xl bg-[#ff3f49] px-5 py-3 text-sm font-black text-white"
        >
          <LogIn class="h-4 w-4" />
          Войти
        </RouterLink>
      ...
      <div class="rounded-[42px] border border-white/10 bg-white/[0.055] p-5 shadow-2xl backdrop-blur sm:p-8 lg:p-10">
        <div class="rounded-[34px] bg-[#07101f] p-7 sm:p-10">
          <div class="flex flex-col justify-between gap-8 lg:flex-row lg:items-start">
            <div>
              <p class="text-xs font-black uppercase tracking-[0.45em] text-[#ff9aa0]">
                карточка грузового места
              </p>

              <h1 class="mt-5 text-4xl font-black tracking-[-0.06em] sm:text-6xl">
                Информация по QR
              </h1>
          ...
          <div
              v-if="loading"
              class="mt-10 flex items-center gap-3 rounded-[28px] border border-white/10 bg-white/[0.05] p-6 text-white/75"
          >
          ...
          <div
              v-else-if="error"
              class="mt-10 rounded-[28px] border border-amber-400/30 bg-amber-400/10 p-6 text-amber-100"
          >
            ...
            <RouterLink
                v-if="!isAuthorized"
                to="/login"
                class="mt-5 inline-flex rounded-2xl bg-white px-5 py-3 text-sm font-black text-slate-950"
            >
              Войти в аккаунт
            </RouterLink>
          </div>
              ...
              <section class="rounded-[32px] border border-emerald-400/25 bg-emerald-400/10 p-6">
                <div class="flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      <p class="text-sm font-black uppercase tracking-[0.25em] text-emerald-200/70">
                        текущий статус
                      </p>

                      <h2 class="mt-2 text-3xl font-black text-emerald-50">
                        {{ statusLabel(cargoItem.status) }}
                      </h2>
                    </div>
                  </div>
                </div>
                ...
                <dl class="mt-8 grid gap-4 sm:grid-cols-2">
                  <div class="rounded-2xl bg-white/[0.06] p-5">
                    <dt class="text-xs uppercase tracking-[0.22em] text-white/45">
                      QR-код
                    </dt>
                  ...
                  <dl class="mt-5 grid gap-4 text-sm">
                    <div>
                      ...
                      <dd class="mt-1 font-black">
                        {{ field('storage_zone_name', 'storage_zone_id', 'storageZoneID') }}
                      </dd>
                    </div>
                    ...
                    <div>
                      ...
                      <dd class="mt-1 font-black">
                        {{ field('gate_name', 'gate_id', 'gateID') }}
                      </dd>
                    </div>
                  </dl>
                </div>
                ...
                <div class="rounded-[28px] border border-white/10 bg-white/[0.055] p-5">
                  ...
                  <dl class="mt-5 grid gap-4 text-sm">
                    <div>
                      ...
                      <dd class="mt-1 font-black">
                        {{ formatDate(field('created_at', 'createdAt')) }}
                      </dd>
                    </div>
                    ...
                    <div>
                      ...
                      <dd class="mt-1 font-black">
                        {{ formatDate(field('updated_at', 'updatedAt')) }}
                      </dd>
                    </div>
                    ...
                    <div>
                      ...
                      <dd class="mt-1 font-black">
                        {{ formatDate(field('received_at', 'receivedAt')) }}
                      </dd>
                    </div>
                    ...
                    <div>
                      ...
                      <dd class="mt-1 font-black">
                        {{ formatDate(field('shipped_at', 'shippedAt')) }}
                      </dd>
                    </div>
                  </dl>
                </div>
            ...
            <section class="mt-5 rounded-[32px] border border-white/10 bg-white/[0.045] p-6">
              ...
              <div v-if="history.length" class="mt-6 grid gap-3">
                <article
                    ...
                    :key="item.id || index"
                    class="rounded-2xl bg-white/[0.055] p-5"
                >
                  <div class="flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
                    <p class="font-black">
                      {{ statusLabel(item.new_status || item.status || item.to_status) }}
                    </p>

                    <p class="text-sm text-white/50">
                      {{ formatDate(item.created_at || item.changed_at) }}
                    </p>
                  </div>

                  <p v-if="item.comment" class="mt-2 text-sm leading-6 text-white/60">
                    {{ item.comment }}
                  </p>
                </article>
              </div>
