...
const navItems = [
  { label: 'Главная', to: '/client', icon: Home },
  { label: 'Мои заявки', to: '/client/orders', icon: ClipboardList },
  { label: 'Создать заявку', to: '/client/orders/new', icon: PlusCircle },
  { label: 'Грузовые места', to: '/client/cargo-items', icon: Boxes },
  { label: 'Профиль', to: '/client/profile', icon: UserRound },
]
...
<template>
  <div class="min-h-screen bg-[#07101f] text-white selection:bg-[#ff4248] selection:text-white">
    <div class="fixed inset-0 -z-10 bg-[radial-gradient(circle_at_5%_5%,rgba(255,66,72,0.18),transparent_34%),radial-gradient(circle_at_92%_12%,rgba(0,166,214,0.22),transparent_32%),linear-gradient(135deg,#160711_0%,#07101f_52%,#073e4f_100%)]"></div>
    ...
    <header class="sticky top-0 z-40 border-b border-white/10 bg-[#07101f]/92 backdrop-blur-xl">
      <div class="mx-auto flex max-w-[1480px] items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-3">
          <RouterLink to="/" class="grid h-12 w-12 place-items-center rounded-2xl bg-[#ff4248] shadow-[0_18px_44px_rgba(255,66,72,0.28)]">
            <PackageCheck class="h-6 w-6 text-white" />
          </RouterLink>
          <div>
            <div class="text-lg font-black tracking-[-0.04em]">Fulfillment Transit</div>
            <div class="text-[10px] font-black uppercase tracking-[0.4em] text-[#ff8e92]">Кабинет клиента</div>
          </div>
        </div>
        ...
        <div class="hidden items-center gap-3 sm:flex">
          <RouterLink to="/client/profile" class="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-2 transition hover:border-[#ff4248]/60 hover:bg-white/10">
        ...
        <div class="ml-auto flex h-full max-w-sm flex-col rounded-[2rem] border border-white/10 bg-[#07101f] p-5 text-white shadow-2xl">
          <div class="flex items-center justify-between">
            <div>
              ...
              <div class="text-[10px] font-black uppercase tracking-[0.35em] text-[#ff8e92]">Кабинет</div>
            </div>
            <button class="grid h-11 w-11 place-items-center rounded-2xl bg-white/10" @click="menuOpen = false">
              <X class="h-5 w-5" />
            </button>
          </div>

          <div class="mt-6 rounded-3xl border border-white/10 bg-white/[0.06] p-4">
            <div class="flex items-center gap-3">
              <div class="grid h-12 w-12 place-items-center rounded-2xl bg-[#ff4248] font-black">{{ initials }}</div>
              <div>
                <div class="font-black">{{ displayName }}</div>
                <div class="text-sm text-white/55">{{ user?.email }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="mt-auto grid gap-3">
            <RouterLink to="/" class="flex items-center justify-center gap-2 rounded-2xl border border-white/15 px-5 py-4 font-black text-white/75" @click="menuOpen = false">
              <ArrowLeft class="h-5 w-5" /> На сайт
            </RouterLink>
            <button class="flex items-center justify-center gap-2 rounded-2xl bg-[#ff4248] px-5 py-4 font-black text-white" @click="logout">
