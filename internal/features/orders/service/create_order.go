...
func (s *OrdersService) CreateOrder(
	...
	case core_domain.HandoverTypePickup:
		if input.SelfDeliveryDate != nil || input.SelfDeliveryTimeFrom != nil || input.SelfDeliveryTimeTo != nil {
		...
		if err := s.ensurePickupCalendarDateAvailable(ctx, input.ReceivingWarehouseID, input.Pickup.PickupDate); err != nil {
			return OrderDTO{}, err
		}
	...
	case core_domain.HandoverTypeSelfDelivery:
		...
		if err := s.ensurePickupCalendarDateAvailable(ctx, input.ReceivingWarehouseID, *input.SelfDeliveryDate); err != nil {
			return OrderDTO{}, err
		}
...
func (s *OrdersService) ensurePickupCalendarDateAvailable(
	ctx context.Context,
	warehouseID int64,
	date string,
) error {
	if s.pickupCalendar == nil {
		return nil
	}
	if err := s.pickupCalendar.EnsureDateAvailable(ctx, warehouseID, date); err != nil {
		return fmt.Errorf("check pickup calendar date: %w", err)
	}
	return nil
}
