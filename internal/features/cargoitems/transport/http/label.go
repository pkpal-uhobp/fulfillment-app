...
func (h *CargoItemsHTTPHandler) GetCargoItemLabel(w http.ResponseWriter, r *http.Request) {
	...
	label, err := h.cargoItemsService.GetCargoItemLabel(
		r.Context(),
		cargoItemID,
		user.ID,
		user.Role,
	...
	if err != nil {
		response.ErrorResponse(err, "get cargo item label")
		return
	}

	response.JSONResponse(CargoItemLabelResponse{Label: label}, http.StatusOK)
}
