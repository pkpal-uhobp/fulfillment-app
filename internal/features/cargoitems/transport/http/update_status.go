...
func (h *CargoItemsHTTPHandler) UpdateCargoItemStatus(w http.ResponseWriter, r *http.Request) {
	...
	var request UpdateCargoItemStatusRequest
	if err := core_http_request.DecodeAndValidateRequest(r, &request); err != nil {
		response.ErrorResponse(err, "invalid update cargo item status request")
		return
	}

	cargoItem, err := h.cargoItemsService.UpdateCargoItemStatus(
		r.Context(),
		cargoItemID,
		user.ID,
		user.Role,
		cargoitems_service.UpdateCargoItemStatusInput{
			Status:  request.Status,
			Comment: request.Comment,
		},
	)
	if err != nil {
		response.ErrorResponse(err, "update cargo item status")
		return
	}
