...
func (h *CargoItemsHTTPHandler) GetCargoItem(w http.ResponseWriter, r *http.Request) {
	...
	cargoItem, err := h.cargoItemsService.GetCargoItem(
		r.Context(),
		cargoItemID,
		user.ID,
		user.Role,
	...
	if err != nil {
		response.ErrorResponse(err, "get cargo item")
		return
	}
